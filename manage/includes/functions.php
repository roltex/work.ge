...
function build_menu($database_link, $parent_id, $section_language, $level = 0, $section_family = 0) {
	...
	$get_sections_query = "SELECT
					...
					`bc_section_types`.`id` = `bc_sections`.`type`
							WHERE
					`bc_sections`.`parent_id` = " . $parent_id . " AND
					`bc_sections`.`family` = " . $section_family . " AND
					`bc_sections`.`language` = '" . $section_language . "'
							ORDER BY
					`bc_sections`.`sort` ASC";
	...
	if ($get_sections_result = DBM::queryData($get_sections_query)) {
		...
		while ($get_sections_data = DBM::fetchObject($get_sections_result)) {
			...
			$retVal .= '<td>' . multiplyString('<i class="glyphicon glyphicon-minus"></i> ', $level) . $get_sections_data->name . '</td>';
			$retVal .= '<td>' . $get_sections_data->type_name . '</td>';
			$retVal .= '<td><a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=edit&pk=' . $get_sections_data->pk . '" class="btn btn-mini btn-warning"><i class="glyphicon glyphicon-edit"></i></a></td>';
			$retVal .= '<td><a href="#" class="btn btn-mini btn-default"><i class="glyphicon glyphicon-list-alt"></i></a></td>';
			$retVal .= '<td><a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=delete&pk=' . $get_sections_data->pk . '" class="btn btn-mini btn-danger" onclick="return confirm(\'დარწმუნებული ხართ, რომ გინდათ გვერდის წაშლა?\');"><i class="glyphicon glyphicon-trash"></i></a></td>';

			$retVal .= '<td>';

			if ($section_has_upper_sort) {
				$retVal .= '<a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=sort&pk=' . $get_sections_data->pk . '&dir=up" class="btn btn-mini btn-info"><i class="glyphicon glyphicon-arrow-up"></i></a>';
			} else {
			...
			if ($section_has_lower_sort) {
				$retVal .= '<a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=sort&pk=' . $get_sections_data->pk . '&dir=down" class="btn btn-mini btn-info"><i class="glyphicon glyphicon-arrow-down"></i></a>';
			} else {
				$retVal .= '<button class="btn btn-mini"><i class="icon-stop icon-white"></i></button>';
			}
...
// Structure Management Build Recursively
function build_statement_menu($database_link, $section_language) {
	...
	if ($get_sections_result = DBM::queryData($get_pages_query)) {
		...
		while ($get_sections_data = DBM::fetchObject($get_sections_result)) {
			...
			$retVal .= '<tr>';
				$retVal .= '<td>' . $get_sections_data->name . '</td>';
				$retVal .= '<td>' . $get_sections_data->company_name . '</td>';
				$retVal .= '<td><a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=edit&pk=' . $get_sections_data->adv_id . '" class="btn btn-mini btn-warning"><i class="glyphicon glyphicon-edit"></i></a></td>';
				$retVal .= '<td><a href="?page=' . $_GET['page'] . '&language=' . $section_language . '&act=delete&pk=' . $get_sections_data->adv_id . '" class="btn btn-mini btn-danger" onclick="return confirm(\'დარწმუნებული ხართ, რომ გინდათ გვერდის წაშლა?\');"><i class="glyphicon glyphicon-trash"></i></a></td>';
...
function structureInCombobox($database_link, $parent_id, $section_language, $level = 0, $current_id, $black_list = '', $family = 0) {
	...
	if ($result = DBM::queryData($query, $database_link)) {
		...
		while ($data = DBM::fetchObject($result)) {
			...
			$ret .= '<label class="checkbox-inline">
												<input type="checkbox" name="menu_type[]" value="' . $data->id . '" class="styled"';
...
function getEditor($field, $value = '', $type = 'small') {
	...
	if ($type == 'full') {
		...
		return '<a href="#file-selector" id="file-selector-button" role="button" class="btn btn-info" data-toggle="modal">Insert File</a>
...
// Sections by Type Combobox
function getCompany($type, $language, $database_link, $current_type = NULL) {
	...
	$query = "SELECT
					...
					`bc_sections`
							WHERE
					`bc_sections`.`language` = '" . $language . "' AND
					`bc_sections`.`family` = '1'
							ORDER BY
					`bc_sections`.`id` ASC";
...
function get_section ($section_id, $language) {
	
	global $database_link;

	$query = "SELECT * FROM `bc_sections` WHERE `id` = '".DBM::escape($section_id)."' AND `language` = '".DBM::escape($language)."'";
...
function get_additional_info ($info_id, $language) {
	
	global $database_link;

	$query = "SELECT * FROM `bc_info_additional` WHERE `info_id` = '".DBM::escape($info_id)."' AND `language` = '".DBM::escape($language)."'";
...
function get_additional_info_byID ($info_id, $language) {
	
	global $database_link;

	$query = "SELECT * FROM `bc_info_additional` WHERE `id` = '".DBM::escape($info_id)."' AND `language` = '".DBM::escape($language)."'";
