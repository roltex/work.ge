...
if (isset($_POST['doEdit'])) {
	...
	if (strlen($name) > 0) {
		...
		$insert_query = "UPDATE `bc_post_type_fields` SET `post_type` = '".(int)$id."' ".$field_query." WHERE `post_type` = '".(int)$id."'";
...
$get_post_type_fields = "SELECT * FROM `bc_post_type_fields` WHERE `post_type` = '".$get_section_data->id."' ORDER BY `id` DESC LIMIT 1";

if($post_fields_result = DBM::queryData($get_post_type_fields, $database_link)) {

	$row = DBM::fetchArray($post_fields_result);
}
?>
...
<form class="form-horizontal" method="post">
	<input type="hidden" name="id" value="<?php echo $get_section_data->id; ?>"/>	
			...
			<div class="form-group">
				...
				<div class="col-lg-9">
					<label class="checkbox-inline">
						<input type="checkbox" name="field_'.$option['field_name'].'" '.is_checked($row[$option['field_name']], 'on').' value="on" class="styled"/>
						'.$option['name'].'
					</label>
				</div>';
				}
