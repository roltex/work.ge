...
//================ get images =====================
	...
	while($image_row = DBM::fetchObject($image_result)){
		...
		$inf_image_arr .= '<tr class="info_images">
							...
							<td><label><input type="radio" name="thumbnail" value="' . $image_row->file . '" required ' . $isThumbnail . '>Thumbnail</input></label>
								<textarea name="edit_photo_desc['.$image_row->id.']" class="img_desc">'.$image_row->name.'</textarea></td>
	...
	$photo = $inf_image_arr;
		...
		$photo = '<tr>
					<td><img src="' . $config['file_path'] .'small_'. $get_info_data->thumbnail . '"/></td>
					<td>
...
if (isset($_POST['doEdit'])) {
	...
	if (isset($_POST['deleteImgs'])){
		...
		foreach($deleted_images_array as $image_file) {
			...
			if(file_exists($config['fpath'].'thumb/'.$image_file))
			@unlink($config['fpath'].'small_'.$image_file);
		}
	} //deleting images end
