...
if (extractPost('doAdd') == $TRANSLATION['save']) {
	...
	if (strlen($title) > 0) {
		...
		$tmp1 = ResizeSemiAbstractTop($src, 119, 95);
		//save image 
		switch ($remote_file)
		{
			case 'image/jpeg':
			...
			if($section_data->style == 1) {
				...
				foreach($additional as $index => $item) {
					...
					$insert_additional = "INSERT INTO 
												`bc_info_additional` 
											SET `info_id` = '".$real_info_id."', 
												`language` = '".DBM::escape($val)."',
	...
	<table class="table">
		...
		<tbody>
			...
			<tr>				
				...
				<td>
					<input type="text" name="fb_url" class="input-xxlarge"/>
				</td>				
			</tr> 
			...
			<tr>				
				...
				<td>
					<input type="text" name="linkedin_url" class="input-xxlarge"/>
				</td>				
			</tr> 
			...
			<tr>				
				...
				<td>
					<input type="text" name="twitter_url" class="input-xxlarge"/>
				</td>				
			</tr> ';
			}
