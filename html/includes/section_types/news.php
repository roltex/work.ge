...
if ($info_id > 0 || $siteData['slug_END']) {
	...
	if( isset($_GET['year']) && (int)extractGet('year') > 1970) {
	...
	if( isset($_GET['month']) && (int)extractGet('month') > 0) {
		
		$month = (int)extractGet('month');
		
		$addQuery .= " AND MONTH(`start_date`) = '".$month."'";
	}
	
	$get_info_query = "SELECT `pk` FROM `bc_info` WHERE
						  `section_id` = " . $siteData['section'] . " AND
						  `language` = '" . $siteData['language'] . "' AND 
						  `published` = 'Y' ".$addQuery."
	...
	if ($get_info_result = DBM::queryData($get_info_query, $database_link)) {
		...
		while ($get_info_data = DBM::fetchObject($get_info_result)) {
			...
			$list_data[] = array (
								...
								'description' => trim_text($get_info_data->description, $language['char_size']*100),
								'thumbnail' => $thumbnail,
	...
	if($siteData['section_style'] == 2)	$smarty->assign('content_file', 'news_page_no_image.tpl');
	else $smarty->assign('content_file', 'news_page.tpl');
}

$smarty->assign('sectionData', $sectionData);
$smarty->assign('post_by_year', $post_by_year);
$smarty->assign('current_year', $year);
$smarty->assign('current_month', $month);
$smarty->assign('current_year', $year);
