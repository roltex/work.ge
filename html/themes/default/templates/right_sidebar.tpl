<div class="col-md-3 shop-sidebar">
	...
	<select class="sidebar-select" name="year">
		<option value="">{$language.year}</option>
		{foreach $post_by_year as $option}
		<option value="{$option.year}" {if $current_year == $option.year}selected="selected"{/if}>{$option.year} ({$option.count})</option>
		{/foreach}
	</select>
	<select class="sidebar-select" name="month">
		<option value="">{$language.month}</option>
		{foreach $language.month_array as $index => $month}
		<option value="{$index}" {if $current_month == $index}selected="selected"{/if}>{$month}</option>
		{/foreach}
	</select>
	</form>
	{/if}
